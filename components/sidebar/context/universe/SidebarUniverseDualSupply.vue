...
export default defineComponent({
  ...
  setup(props) {
    ...
    const balance0 = computed(() => getBalanceByKey(token0.value?.key));

    const symbol1 = computed(() => token1.value?.symbol);
    const decimals1 = computed(() => token1.value?.decimals);
    const balance1 = computed(() => getBalanceByKey(token1.value?.key));
    ...
    async function cast() {
      ...
      const vaultAddress = selectedVault.value.vaultProxyAddress ?? props.vault;

      spells.add({
        //@ts-ignore
        connector: "UNIVERSE-A",
        method: "deposit",
        args: [vaultAddress, ...amounts, [0, 0], [0, 0]]
      });
