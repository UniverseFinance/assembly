  ...
  >
    ...
    <div class="flex items-center h-20">
      <div class="flex mr-4 -space-x-3 overflow-hidden">
        ...
        >
          <img class="w-full h-full object-cover" :src="token0Icon" />
        </div>
        <IconCurrency
          v-else
          :currency="token0Key"
      ...
      <div class="flex flex-col flex-grow">
        <div class="font-medium text-lg leading-8 whitespace-no-wrap">
          {{ formatDecimal(token0Supply) }} {{ symbol0 }}
        </div>
        <div class="flex h-4 whitespace-no-wrap">
          <span class="text-sm font-medium text-[#9FB0C9]">{{
            formatUsd(token0SupplyUsd)
          }}</span>
          <Info
            :text="`${formatUsd(price0InUsd, 2)}/${symbol0}`"
            icon="price"
            class="ml-1"
          />
        </div>
        <div class="font-medium text-lg leading-8 whitespace-no-wrap">
          {{ formatDecimal(token1Supply) }} {{ symbol1 }}
        </div>
        <div class="flex h-4 whitespace-no-wrap">
          <span class="text-sm font-medium text-[#9FB0C9]">{{
            formatUsd(token1SupplyUsd)
          }}</span>
          <Info
            :text="`${formatUsd(price1InUsd, 2)}/${symbol1}`"
            icon="price"
            class="ml-1"
          />
        </div>
      ...
      <div class="text-right mr-4 hidden md:block">
        <p class="leading-8  ">
          <span class="text-base font-medium">{{ untReward }}</span>
      ...
      <div class="ml-auto text-right">
        <p class="leading-8	 text-lg font-bold text-green-400">
        ...
        <div class="h-4 flex">
          ...
          <Info
            class="ml-1 h-3"
            text="Trading fees earned in Uniswap V3 pool"
          />
        </div>
      </div>
      <!--
...
export default defineComponent({
  props: {
    ...
    token0Key: { type: String, required: true },
    token0Icon: { type: String, required: false },
    token1Key: { type: String, required: true },
    token1Icon: { type: String, required: false },
    vault: { type: String, default: null },
    token0Supply: { type: String, required: true },
    token0SupplyUsd: { type: String, required: true },
    token1Supply: { type: String, required: true },
    token1SupplyUsd: { type: String, required: true },
    untReward: { type: Number, default: 0 },
    price0InUsd: { type: String, default: "0" },
    price1InUsd: { type: String, default: "0" },
  ...
  setup(props) {
    ...
    const symbol0 = computed(
      () => getTokenByKey(props.token0Key)?.symbol || props.token0Key
    );
    const symbol1 = computed(
      () => getTokenByKey(props.token1Key)?.symbol || props.token1Key
    );

    function showSupply() {
      app.router.push({
        hash: `dual-supply?vault=${props.vault}`
      });
    }

    function showWithdraw() {
      app.router.push({
        hash: `dual-withdraw?vault=${props.vault}`
      });
    }
